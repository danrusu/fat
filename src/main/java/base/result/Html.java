...
public class Html {
    ...
    public static String tag(String tagName, String... attributes) {
        ...
        return String.join(
                ...
                "\n<", tagName, List.of(attributes).stream().reduce("", (acc, x) -> acc + " " + x), ">\n"
                );
    }

    private static Function<Entry<String, String>, String> entryAtributeToString = entry -> entry.getKey() + "=\""
            + entry.getValue() + "\"";

    public static String tag(String tagName, Map<String, String> attributes) {

        return "<" + tagName + " " + attributes.entrySet().stream().map(entryAtributeToString).collect(joining(" "))
                + ">";
    }
    ...
    // e.g. attributes = [ "width=\"100\"", "title=\"age\"" ]
    public static String tagFromText(String tagName, String text, Function<String, String> stringTransformFunction,
            String... attributes) {
        ...
        return (text != null) ?
                ...
                String.join(
                        "", tag(tagName, attributes), stringTransformFunction.apply(text
                                ), endTagOnNewLine(
                                        tagName
                                        )
                        )
    ...
    public static String detailsHead() {
        ...
        return head(
                ...
                new String[] { REPORTS_CSS, REPORTS_RESPONSIVE },
    ...
    public static String createTableColumns(String... columnsText) {
        ...
        return List.of(columnsText).stream().reduce("", (acc, col) -> acc + Html.tagFromText("td", col));
    }

    public static String createTableRow(String... columnsText) {

        return Html.tagFromText(
                "tr", List.of(columnsText
                        ).stream().reduce("", (acc, x) -> acc + Html.tagFromText("td", x)));
    }

    public static String createTableHeaderRow(String... columnsText) {

        return Html.tagFromText("tr", List.of(columnsText).stream().reduce("", (acc, x) -> acc + tagFromText("th", x)));
    }
