...
import static main.java.base.failures.Failure.failureStackToString;
import static main.java.base.failures.ThrowablesWrapper.supplyAndMapThrowableToFailure;
import static main.java.base.selenium.Browser.CHROME;
import static main.java.base.selenium.Browser.FIREFOX;
import static main.java.base.selenium.Browser.IE;
import static main.java.utils.FileUtils.copyFile;
...
import main.java.utils.StreamUtils;
...
public class Driver {
    ...
    public static Map<Long, String> getJSErrorsFromBrowserLog(){
        ...
        if (false == isChromeDriver()) {
            log("BROWSER_JS_ERROR: Java script errors not checked for this browser.");
            return Collections.emptyMap();
        }

        LogEntries logEntries = driver.manage().logs().get(LogType.BROWSER);
        if(logEntries == null){
            log("BROWSER_JS_ERROR: Could not get the Chrome LogEntries.");
            return Collections.emptyMap();
        }
        
        List<LogEntry> severeLogEntries = logEntries.filter(Level.SEVERE);                   
        
        Map<Long, String> jsErrorsByTimestamp = severeLogEntries.stream()
        
            .peek(logInfoForLogEntry)
            
            .collect(toMap(
                    LogEntry::getTimestamp,
                    getLogEntryInfo,
                    StreamUtils.mergeByKeepingLastStringFunction,
                    TreeMap::new));
       
        return jsErrorsByTimestamp;
    }
    
    
    private static Consumer<? super LogEntry> logInfoForLogEntry = logEntry -> logAll(
            "BROWSER_JS_ERROR :", 
            logEntry.getMessage(), " | ", 
            logEntry.getLevel().getName());
    
    
    private static Function<? super LogEntry, ? extends String> getLogEntryInfo = logEntry -> 
