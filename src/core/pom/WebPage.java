...
public class WebPage {
    ...
    /**
     * Click web element via java script.
     * 
     * @param by - web element locator
     */
    public void jsClick(By by){

        log("Click (javascript): " + by);
        ...
        if (we.isEnabled()){
            ...
            Assert.fail("Element " + by + "is disabled!");
        }
    }
    ...
    public void jsTriggerEvent(WebElement element, String ...domEvents) {
        ...
        executeScript(script, element, domEvents);
    }



    public void jsTriggerChange(WebElement element){
        jsTriggerEvent(element, "change");
    }


    public void jqTriggerEnter(WebElement element) {
        executeScript("var e = $.Event( \"keypress\", { which: 13 } ); $(arguments[0]).trigger(e)", element);
        waitForAjaxFinish();
    }
    ...
    /**
     * Add space in a string after each segmentLength characters.
     * This is used for splitting the search box string in eFlow.
     * (Elastic search supports search by 16 chars length strings segments)
     * 
     * @param text - string to change
     * @param segmentLength - insert space after each segmentLength chars
     * @return
     */
    public String getSearchText(String text, int segLength) {
        StringBuilder finalString = new StringBuilder();
        int segmentLength = segLength - 1;

        IntStream.rangeClosed(1, text.length()/segmentLength).forEach(
                i -> {
                    final String substr = text.substring((i-1)*segmentLength, segmentLength+(i-1)*segmentLength);
                    finalString.append(substr).append(" ");
                }
                );

        String remainingText = text.substring(text.length()-text.length()%segmentLength);
        // do not add if only one char remains - the search won't work0
        if (remainingText.length()>1){
            finalString.append(remainingText);
        }
        log("getSearchText(" + segmentLength + "): \"" 
    ...
    public static String byToString(By by){
        ...
        int pos = s.indexOf(":");
        return s.substring(pos+1, s.length()).trim();
    }
     ...
     * @return
     */
    public static By replace(By selector, String toReplace, String replacement){
        String s = byToString(selector).replace(toReplace, replacement);
    ...
    public By addPathToCssSelector(By selector, String path){
        return By.cssSelector(byToString(selector) + path);
    }



    public By addPathToXpathSelector(By selector, String path){
        return By.xpath(byToString(selector) + path);
    }



    private String getEvalXpathJavascript(){
        return "function eval(xpath){ "
     ...
     */
    public void addJsScriptToDOM(String jsScript){
        ...
        //log("Javascript code: " + jsScriptAdder);
        executeScript(jsScriptAdder);
    }
     ...
     */
    public void addEvalXpathJavascriptToDOM(){
        log("Add eval(xpath) to DOM"); 
        addJsScriptToDOM(getEvalXpathJavascript());
    }
    ...
    public boolean selectJs(By checkboxLocator, boolean selected) {
        if (selected) {
            return jsCheck(checkboxLocator);
        }
        else {
            return jsUncheck(checkboxLocator);
        }
    }
    ...
    public boolean jsCheck(By checkboxLocator){
        log("Check the checkbox via js" + checkboxLocator);
        WebElement checkbox = driver.findElement(checkboxLocator);
        if (checkbox.isSelected()){
            log("Already checked! No action needed.");
        }
    ...
    public boolean jsUncheck(By checkboxLocator){
        log("Uncheck the checkbox via js " + checkboxLocator);
        WebElement checkbox = driver.findElement(checkboxLocator);
        if (!checkbox.isSelected()){
            log("Already unchecked! No action needed.");
        }
    ...
    public boolean isElementFound(By by, long timeout){
    ...
    public boolean isElementFound(By by){
        return isElementFound(by, Driver.getDefaultWait()*1000);
    }
    ...
    public String jsAjaxGet(String url, 
            String ajaxProperties){
        return jsAjax(HttpVerb.GET, url, ajaxProperties, "");
    }



    public String jsAjaxPost(String url, 
            String ajaxProperties,
            String formDataJs){
        return jsAjax(HttpVerb.POST, url, ajaxProperties, formDataJs);
    }

    public String getAjaxPostScript(String url, String ajaxProperties){
        String script = "\n$.ajax({"
